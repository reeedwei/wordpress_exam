	...
	if ( ( ( web_log_categorized_blog() && $categories_list ) || $tags_list ) || get_edit_post_link() ) {
		...
		echo '<footer class="entry-footer">';
			...
			if ( 'post' === get_post_type() ) {
				if ( ( $categories_list && web_log_categorized_blog() ) || $tags_list ) {
					echo '<span class="cat-tags-links">';

						// Make sure there's more than one category before displaying.
						if ( $categories_list && web_log_categorized_blog() ) {
							echo '<span class="cat-links">' . web_log_get_svg( array( 'icon' => 'folder-open' ) ) . '<span class="screen-reader-text">' . esc_html__( 'Categories', 'web-log' ) . '</span>' . esc_html( $categories_list) . '</span>';
						}

						if ( $tags_list && ! is_wp_error( $tags_list ) ) {
							echo '<span class="tags-links">' . web_log_get_svg( array( 'icon' => 'hashtag' ) ) . '<span class="screen-reader-text">' . esc_html__( 'Tags', 'web-log' ) . '</span>' . esc_html( $tags_list) . '</span>';
						}
