 ...
 */
final class Pagoda_Press_Webfonts_Local{
	...
	public function add_styles() {
		...
		foreach( $hosted_fonts as $family ){
			...
			$css .= Pagoda_Press_Google_Local::init( $family )->get_css();
		}

		// If we've got CSS, add to the footer.
		if ( $css ) {
			echo '<style id="pagoda-press-local-webfonts">' . $css . '</style>'; // WPCS: XSS ok.
		}
	}
}